 ...
 * `rate_limit_event`, emitted when unified usage-limit info changes, and
 * `command_lifecycle` frames, which report each uuid-stamped message's terminal
 * state — queued/started/completed) that have no chat representation. Shared by every stdout parsing path so a new benign type
 ...
 */
export const KNOWN_IGNORED_MESSAGE_TYPES = new Set<string>([
  'rate_limit_event',
  'command_lifecycle',
]);
